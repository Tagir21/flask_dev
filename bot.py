...
def random_horoscope_f():
        horo = Horoscope.query.all()
        print(horo[randint(horo[0].id - 1, horo[-1].id - 1)].text)
        return horo[randint(horo[0].id - 1, horo[-1].id) - 1].text

# Обработчик очереди
# Пушим контекст для обработчика контекста
def run_schedule_tasks(app_context):
    app_context.push()
    while True:
...
def every_hour(message):
    ...
    if (chat_id in users_shed) and (len(schedule.get_jobs(str(chat_id))) > 0):
        ...
        schedule.every().minute.do(lambda : bot.send_message(chat_id, random_horoscope_f())).tag(str(chat_id))
...
if __name__ == '__main__':
    scheduleThread = Thread(target=run_schedule_tasks, args=[app.app_context()])
